...
public class PhoneBookManager {
	...
	void showGroupData(int type) {
		...
		case Menu.UNIV:
			...
			for(int i=0; i<pBooks.size(); i++) {
				...
				if(pBooks.get(i) instanceof PhoneUnivInfor) {
					//pBooks[i].showData();
					pBooks.get(i).showData();
					search = true;
				}
		...
		case Menu.COMP:
			for(int i=0; i<pBooks.size(); i++) {
				if(pBooks.get(i) instanceof PhoneCompanyInfor) {
					pBooks.get(i).showData();
					search = true;
				}
			}
		...
		case Menu.CLUB:
			for(int i=0; i<pBooks.size(); i++) {
				if(pBooks.get(i) instanceof PhoneClubInfor) {
					pBooks.get(i).showData();
					search = true;
				}
			}
	...
	void editInfo() {
		...
		if(searchIndex<0) {
			...
			String editName = pBooks.get(searchIndex).name;
			...
			else if(pBooks.get(searchIndex) instanceof PhoneClubInfor) {
